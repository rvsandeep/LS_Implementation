...
public class LsImplementation implements LsInterface {
	...
	/**
	 * BitVector assigns a precedence bit to each attribtutes, helps to
	 * determine which operation to do first. operations involving getting the
	 * directories and subfiles to operate on should be done first sorting is
	 * done later and the attributes like size are added later to ouput
	 * 
	 */
	class BitVector {
	...
	/**
	 * sorts the bitVector in ascending order indicating the first operation to
	 * be the most preferred one
	 */
	 ...
	 * @param c
	 * @param index
	 *            adds the bit to bitVector, the index is used after sorting
	 *            bitVector based on bit to set the Attribtues
	 */
	...
	/**
	 * @param c
	 * @return based on the attribute returns the precedence of the operation
	 *         operations like sorting are given same precedence since the order
	 *         of the input dominates in such case
	 */
	...
	public List<String> performOperation() throws IOException {
		...
		toPrint = Operation.performOperation();
		return toPrint;
	}
}
